...
function getPrimaryImageUrl(
	images: CheckoutVariant["product"]["images"],
	format: ProductFormat,
) {
	const preferredType = format === "REFILL" ? "REFILL" : "MAIN";

	return (
		images.find((image) => image.imageType === preferredType)?.url ??
...
function idempotentReplayResult(
	...
	if (input.userId === null) {
		throw new HttpError(
			409,
			"An order already exists for this idempotency key",
		);
	}
...
function buildLineItems(
	...
	requestedItems: MergedOrderItem[],
) {
	...
	return requestedItems.map((requestedItem) => {
		...
		if (!variant.product.isActive) {
			throw new HttpError(
				409,
				`Product for SKU ${variant.sku} is unavailable`,
			);
		}
	...
	return prisma.$transaction(
		async (tx) => {
			...
			if (input.promotionCode !== null) {
				...
				if (subtotal.lt(promotion.minSubtotal)) throw new HttpError(409, `Promotion requires a minimum subtotal of ${promotion.minSubtotal.toNumber()} EUR`);
