...
export function mapAdminProductSummary(product: AdminProductListRecord) {
			...
			prices.length === 0
				...
				: decimalToNumber(
						prices.reduce((lowest, price) =>
							price.lessThan(lowest) ? price : lowest,
						),
					),
		variantCount: product.variants.length,
		totalStock: stockValues.reduce((sum, stock) => sum + stock, 0),
		lowStockVariantCount: stockValues.filter(
...
export function mapAdminProductDetail(product: ProductRecord) {
	return {
		id: product.id,
		slug: product.slug,
		name: product.name,
