...
async function normalizedCartItems(items: CartReplaceInput["items"]) {
  const quantities = new Map<string, number>();
  for (const item of items) quantities.set(item.variantId, Math.min(20, (quantities.get(item.variantId) ?? 0) + item.quantity));
...
async function replaceCartRecord(userId: string, items: CartReplaceInput["items"]) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (normalized.length) await tx.cartItem.createMany({ data: normalized.map((item) => ({ ...item, cartId: cart.id })) });
  });
}
...
async function readCart(userId: string) {
  ...
  const items = (cart?.items ?? []).map((item) => { const product = mapProduct(item.product); const variant = product.variants.find((candidate) => candidate.id === item.variantId); return variant ? { id: `${product.id}:${variant.id}`, productId: product.id, variantId: variant.id, quantity: item.quantity, product, variant } : null; }).filter((item): item is NonNullable<typeof item> => item !== null);
...
async function readWishlist(userId: string) {
...
async function replaceWishlistRecord(userId: string, productIds: string[]) {
  const unique = [...new Set(productIds)];
  const products = await prisma.product.findMany({ where: { id: { in: unique }, isActive: true }, select: { id: true } });
  await prisma.$transaction(async (tx) => { await tx.wishlistItem.deleteMany({ where: { userId } }); if (products.length) await tx.wishlistItem.createMany({ data: products.map(({ id: productId }) => ({ userId, productId })) }); });
}

export async function getContinuity(userId: string) { const [cart, wishlist] = await Promise.all([readCart(userId), readWishlist(userId)]); return { data: { cart, wishlist } }; }
export async function mergeContinuity(userId: string, input: ContinuityMergeInput) {
  const current = await readCart(userId);
  const merged = new Map(current.items.map((item) => [item.variantId, item.quantity]));
  for (const item of input.cartItems) merged.set(item.variantId, Math.min(20, (merged.get(item.variantId) ?? 0) + item.quantity));
...
export async function replaceCart(userId: string, input: CartReplaceInput) { await replaceCartRecord(userId, input.items); return { data: await readCart(userId) }; }
export async function replaceWishlist(userId: string, input: WishlistReplaceInput) { await replaceWishlistRecord(userId, input.productIds); return { data: await readWishlist(userId) }; }
