import {
	...
	findAdminOrderPage,
	updateAdminOrderStatusAtomically,
	updateAdminPaymentStatusAtomically,
	type AdminOrderTransitionState,
...
import {
	isOrderStatusTransitionCompatible,
	isPaymentStatusTransitionCompatible,
...
function mapOrderTransitionError(error: unknown): never {
...
function validateOrderStatusTransition(
	order: AdminOrderTransitionState,
	nextStatus: AdminOrderStatusUpdateInput["status"],
) {
	if (
		isOrderStatusTransitionCompatible(
			{
	...
	if (
		...
		throw new HttpError(
			409,
			`Payment must be marked paid before this order can be ${nextStatus.toLowerCase()}.`,
		);
	}
	...
	if (nextStatus === "REFUNDED" && order.paymentStatus !== "REFUNDED") {
		throw new HttpError(
			409,
			"Payment must be marked refunded before the order can be refunded.",
		);
	}
...
function validatePaymentStatusTransition(
	order: AdminOrderTransitionState,
	nextStatus: AdminPaymentStatusUpdateInput["paymentStatus"],
) {
	if (
		isPaymentStatusTransitionCompatible(
			{
	...
	if (
		...
		throw new HttpError(
			409,
			"Payment can only be marked refunded after delivery or cancellation.",
		);
	}

	throw new HttpError(
		409,
		`Payment status cannot move from ${order.paymentStatus} to ${nextStatus} for this order.`,
	);
}
...
export async function setAdminOrderStatus(
	...
	try {
		const order = await updateAdminOrderStatusAtomically(
			orderNumber,
			input.status,
			(current) => validateOrderStatusTransition(current, input.status),
		);
...
export async function setAdminPaymentStatus(
	orderNumber: string,
	input: AdminPaymentStatusUpdateInput,
) {
	try {
		const order = await updateAdminPaymentStatusAtomically(
			orderNumber,
			input.paymentStatus,
			(current) =>
				validatePaymentStatusTransition(current, input.paymentStatus),
		);
