...
export async function getPublishedArticle(slug: string) {
  ...
  if (!row) throw new HttpError(404, "Journal article not found");
...
export async function createArticle(input: JournalArticleInput) {
...
export async function updateArticle(id: string, input: JournalArticleUpdateInput) {
  const current = await prisma.journalArticle.findUnique({ where: { id } });
  if (!current) throw new HttpError(404, "Journal article not found");
  ...
  const data = Object.fromEntries(Object.entries(input).filter(([, value]) => value !== undefined)) as Prisma.JournalArticleUpdateInput;
