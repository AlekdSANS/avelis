...
export async function buildSitemapXml() {
	...
	const entries: SitemapEntry[] = [
		...staticEntries,
		...products.map((item) => ({ path: `/products/${item.slug}`, lastModified: item.updatedAt, changeFrequency: "weekly" as const, priority: 0.8 })),
		...collections.map((item) => ({ path: `/collections/${item.slug}`, lastModified: item.updatedAt, changeFrequency: "weekly" as const, priority: 0.8 })),
		...articles.map((item) => ({ path: `/journal/${item.slug}`, lastModified: item.updatedAt, changeFrequency: "monthly" as const, priority: 0.6 })),
	];
