import { countAdminProducts, createAdminProductRecord, deactivateAdminProduct, findAdminProductById, findAdminProductReferenceNotes, findAdminProductReferenceCollections, findAdminProducts, updateAdminProductStatus, updateAdminProductRecord, } from "../repositories/adminProductRepository.js";
import { mapAdminProductDetail, mapAdminProductSummary, } from "../utils/adminProductMapper.js";
...
function mapAdminProductPersistenceError(error) {
    if (error instanceof Error) {
        switch (error.message) {
            ...
            case "ADMIN_PRODUCT_INACTIVE_NOTE":
                throw new HttpError(409, "Inactive fragrance notes cannot be added to a product");
            case "ADMIN_PRODUCT_INACTIVE_COLLECTION":
                throw new HttpError(409, "Inactive collections cannot be added to a product");
            ...
            case "ADMIN_PRODUCT_INVALID_VARIANT_ID":
                throw new HttpError(409, "One or more variant IDs do not belong to this product");
            case "ADMIN_PRODUCT_INVALID_IMAGE_ID":
                throw new HttpError(409, "One or more image IDs do not belong to this product");
        }
    }
...
export async function setAdminProductStatus(id, input) {
    try {
...
export async function softDeleteAdminProduct(id) {
    try {
        ...
        return {
            data: mapAdminProductSummary(product),
            message: "Product deactivated. Storefront visibility is disabled and historical order data is preserved.",
        };
    }
    catch (error) {
        if (isMissingRecordError(error)) {
...
export async function updateAdminProduct(id, input) {
    try {
        const { product, removedStorageKeys } = await updateAdminProductRecord(id, normalizeUpdateInput(input));
        const cleanupResults = await Promise.allSettled(removedStorageKeys.map((storageKey) => imageStorage.deleteProductImage(storageKey)));
        if (process.env.NODE_ENV !== "production" &&
            cleanupResults.some((result) => result.status === "rejected")) {
            console.error("One or more detached product image files could not be removed");
        }
        return {
            data: mapAdminProductDetail(product),
        };
