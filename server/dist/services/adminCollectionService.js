import { archiveAdminCollectionRecord, countAdminCollections, createAdminCollectionRecord, findAdminCollectionById, findAdminCollectionBySlug, findAdminCollections, updateAdminCollectionRecord, } from "../repositories/adminCollectionRepository.js";
...
function mapCollectionListItem(collection) {
    return {
        ...collection,
...
function mapCollectionPersistenceError(error) {
    if ((error instanceof Error && error.message === "ADMIN_COLLECTION_NOT_FOUND") ||
...
async function ensureUniqueSlug(slug, excludingId) {
...
export async function listAdminCollections(query) {
...
export async function createAdminCollection(input) {
    ...
    if (slug.length === 0)
        throw new HttpError(400, "Collection slug is required");
    ...
    try {
        return {
            data: mapCollectionDetail(await createAdminCollectionRecord({ ...input, slug })),
        };
    }
    catch (error) {
...
export async function updateAdminCollection(id, input) {
    ...
    assertPublishable({
        ...
        cardImageUrl: input.cardImageUrl === undefined
            ...
            : input.cardImageUrl,
        productIds: input.productIds ?? current.products.map(({ product }) => product.id),
    ...
    try {
        ...
        const detachedStorageKeys = previousImageUrls
            ...
            .filter((key) => key !== null);
        await Promise.allSettled([...new Set(detachedStorageKeys)].map((storageKey) => deleteAdminProductUpload({ storageKey })));
        return {
...
export async function archiveAdminCollection(id) {
    try {
        return {
            data: mapCollectionDetail(await archiveAdminCollectionRecord(id)),
            message: "Collection archived. Product records and assignments were preserved.",
        };
    }
    catch (error) {
        mapCollectionPersistenceError(error);
...
//# sourceMappingURL=adminCollectionService.js.map