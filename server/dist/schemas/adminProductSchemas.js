import { z } from "zod";
import { ADMIN_PRODUCT_DEFAULT_PAGE_SIZE, ADMIN_PRODUCT_MAX_PAGE_SIZE, ADMIN_PRODUCT_VOLUME_BY_FORMAT, } from "../config/adminProduct.js";
...
function validateVariant(variant, context) {
    const allowedVolumes = ADMIN_PRODUCT_VOLUME_BY_FORMAT[variant.format];
    if (!allowedVolumes.includes(variant.volumeMl)) {
        context.addIssue({
            code: "custom",
...
const imageFields = {
    url: z
        ...
        value.startsWith("http://"), "Image URL must be an absolute URL or root-relative path"),
...
const noteRelationSchema = z
    ...
    .strict();
function validateVariants(variants, context) {
...
function validateImages(images, context) {
...
function validateRelations(notes, collectionIds, context) {
...
export const adminProductUpdateSchema = z
    ...
    else if (input.collectionIds !== undefined &&
...
//# sourceMappingURL=adminProductSchemas.js.map