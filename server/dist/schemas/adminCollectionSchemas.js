...
const collectionSlugSchema = z
    ...
    .min(1, "Collection slug is required")
    .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/));
const nullableText = (max) => z.union([z.string().trim().min(1).max(max), z.null()]).optional();
const nullableImageUrlSchema = z
    ...
    z
        ...
        value.startsWith("http://"), "Image URL must be absolute or root-relative"),
    z.null(),
])
    .optional();
...
//# sourceMappingURL=adminCollectionSchemas.js.map