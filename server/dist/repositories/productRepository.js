...
function hasVariantFilters(query) {
    return ((query.format?.length ?? 0) > 0 ||
        (query.volume?.length ?? 0) > 0 ||
        query.inStock === true ||
        query.minPrice !== undefined ||
        query.maxPrice !== undefined);
}
export function buildVariantWhere(query) {
    const where = {};
...
export function buildProductWhere(query) {
    const where = {
        isActive: true,
    };
    const and = [];
    if (query.search !== undefined) {
        and.push(...query.search
            .split(/[,\s]+/)
            .map((term) => term.trim())
            .filter(Boolean)
            .map(buildTextSearch));
...
export async function findFeaturedProducts(limit) {
    return prisma.product.findMany({
        where: {
            isActive: true,
            isFeatured: true,
...
export async function findRelatedSource(productId) {
    return prisma.product.findFirst({
        where: {
            id: productId,
            isActive: true,
