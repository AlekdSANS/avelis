...
import { adminProductListSelect, } from "../utils/adminProductMapper.js";
import { productSelect } from "../utils/productMapper.js";
const insensitive = "insensitive";
export function buildAdminProductWhere(query) {
    const and = [];
...
function getAdminProductOrderBy(sort) {
    switch (sort) {
        case "oldest":
...
async function findAggregateSortedProductIds(query, where) {
    const skip = (query.page - 1) * query.limit;
    const isPriceSort = query.sort === "price-asc" || query.sort === "price-desc";
    const direction = query.sort === "price-asc" || query.sort === "stock-asc"
        ? "asc"
        : "desc";
    if (isPriceSort) {
...
export function countAdminProducts(query) {
    return prisma.product.count({
        where: buildAdminProductWhere(query),
    });
}
export async function findAdminProducts(query) {
    const where = buildAdminProductWhere(query);
    const usesAggregateSort = query.sort.startsWith("price-") || query.sort.startsWith("stock-");
    if (usesAggregateSort) {
        ...
        return products.sort((left, right) => (positionById.get(left.id) ?? 0) - (positionById.get(right.id) ?? 0));
    }
...
export function findAdminProductById(id) {
...
export function updateAdminProductStatus(id, input) {
    const data = {};
...
async function validateReferencedRelations(tx, notes, collectionIds, productId) {
    if (notes !== undefined) {
        const noteIds = [...new Set(notes.map((note) => note.noteId))];
        const existingNotes = noteIds.length === 0
            ? []
        ...
        if (inactiveIds.length > 0) {
            const existingRelations = productId === undefined
                ? []
            ...
            const existingRelationKeys = new Set(existingRelations.map((relation) => `${relation.noteId}:${relation.type}`));
            const includesNewInactiveRelation = notes
                .filter((note) => inactiveIdSet.has(note.noteId))
                .some((note) => !existingRelationKeys.has(`${note.noteId}:${note.type}`));
    ...
    if (collectionIds !== undefined) {
        const existingCollections = collectionIds.length === 0
            ? []
        ...
        if (inactiveIds.length > 0) {
            const existingRelationCount = productId === undefined
                ? 0
...
async function validateUniqueFields(tx, params) {
...
function createProductData(input) {
    return {
        name: input.name,
        slug: input.slug,
...
export function createAdminProductRecord(input) {
...
function buildProductScalarUpdate(input) {
    const data = {
        updatedAt: new Date(),
    };
...
async function reconcileVariants(tx, productId, variants, currentVariantIds) {
    const preservedIds = variants.flatMap((variant) => variant.id === undefined ? [] : [variant.id]);
...
async function reconcileImages(tx, productId, images, currentImages) {
    const preservedIds = images.flatMap((image) => image.id === undefined ? [] : [image.id]);
    ...
    const preservedStorageKeys = new Set(images.flatMap((image) => image.storageKey === undefined ? [] : [image.storageKey]));
    return currentImages.flatMap((image) => image.storageKey !== null && !preservedStorageKeys.has(image.storageKey)
        ? [image.storageKey]
        : []);
}
export function updateAdminProductRecord(id, input) {
    return prisma.$transaction(async (tx) => {
        ...
        const currentVariantIds = currentProduct.variants.map((variant) => variant.id);
        let removedStorageKeys = [];
        await validateReferencedRelations(tx, input.notes, input.collectionIds, id);
        await validateUniqueFields(tx, {
        ...
        if (input.variants !== undefined) {
            await reconcileVariants(tx, id, input.variants, currentVariantIds);
        }
        if (input.images !== undefined) {
            removedStorageKeys = await reconcileImages(tx, id, input.images, currentProduct.images);
        }
...
//# sourceMappingURL=adminProductRepository.js.map