...
export function getLocalCartItems() { return useCartStore.getState().items; }
export function replaceLocalCart(items: CartItem[]) { useCartStore.setState({ items, hasHydrated: true }); }
export function clearLocalCart() { useCartStore.getState().clearCart(); }
export function subscribeLocalCart(listener: (items: CartItem[]) => void) { return useCartStore.subscribe((state, previous) => { if (state.items !== previous.items) listener(state.items); }); }
