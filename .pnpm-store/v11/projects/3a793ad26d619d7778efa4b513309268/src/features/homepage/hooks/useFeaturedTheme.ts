...
function selectSessionTheme(): HomepageTheme {
	...
	try {
		const requestedThemeId = new URLSearchParams(window.location.search).get(
			"theme",
		);
		const requestedTheme = homepageThemes.find(
			(theme) => theme.id === requestedThemeId,
		);

		if (requestedTheme) {
			window.sessionStorage.setItem(STORAGE_KEY, requestedTheme.id);
