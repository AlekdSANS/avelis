...
export function ProductMerchandisingPanel({ productId, productName }: { productId: string; productName: string }) {
  ...
  const user = useCurrentUser(); const [email, setEmail] = useState(user.data?.email ?? ""); const [saved, setSaved] = useState(false); const [error, setError] = useState("");
  ...
  return data.sampleAvailable || data.lowStock || data.outOfStock ? <aside className={styles.notices} aria-label="Availability details">
