...
const productCardSource = readFileSync(
  new URL("../../components/commerce/ProductCard/ProductCard.tsx", import.meta.url),
  "utf8",
);
const merchandisingPanelSource = readFileSync(
  new URL(
    "../merchandising/ProductMerchandisingPanel.tsx",
    import.meta.url,
  ),
  "utf8",
);
...
test("product pages do not render inconsistent curated recommendations", () => {
  assert.doesNotMatch(merchandisingPanelSource, /Continue the composition/);
  assert.doesNotMatch(merchandisingPanelSource, /curated_recommendations/);
});
